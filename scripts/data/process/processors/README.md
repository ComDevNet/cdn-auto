...
- [modulegaze.py](./modulegaze.py) — ModuleGaze access/session logs from `/var/log/modulegaze`
...
- Inputs: v4 expects Apache combined lines; v5 expects JSON lines with a message; v3 expects JSON with extended D-Hub module paths; v6 expects JSON with module paths similar to v3 but stored in /var/log/oc4d; ModuleGaze expects active `.log` files or daily `.log.zip` archives
...
- Module extraction: dhub.py, log-v6.py, and modulegaze.py handle `/uploads/modules/[id]/[module-name]`, `/modules/[id]/[module-name]`, and `/uploads/other-modules/[module-name]` path formats
