...
def compute_window(
    ...
    # Near-realtime: today-so-far through now (includes current incomplete activity).
    # Bucket stamp still rotates every RUN_INTERVAL so harvest cadence stays visible, but the
    # filter must not drop rows from earlier buckets — ModuleGaze ingest SETs day maps from
    # each CSV, so the file needs the full day-to-now picture (not only the current 15m slice).
    if schedule_type in ("near_realtime", "rolling"):
        ...
        bucket_start = epoch + timedelta(seconds=bucket_start_sec)
        start = datetime.combine(now.date(), time.min).replace(microsecond=0)
        ...
        return Window(
            ...
            file_stamp=f"nr_{bucket_start.strftime('%Y%m%d_%H%M')}_{interval}s",
            label=(
                f"Near-realtime today→now "
                f"({start.strftime('%Y-%m-%d %H:%M:%S')} to {end.strftime('%Y-%m-%d %H:%M:%S')}; "
                f"stamp {bucket_start.strftime('%H:%M')}/{interval}s)"
            ),
        )
